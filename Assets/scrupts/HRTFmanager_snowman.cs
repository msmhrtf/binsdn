...
public class HRTFmanager_snowman : MonoBehaviour
{
    ...
    public float omega_low = 500; // snowman cutoff freq low
    public float omega_high = 3000; // snowman cutoff freq high
    public int idx_low, idx_high; // fft indexes for omega low and high
    //public List<AForge.Math.Complex[][]> hrtf_snowman_nodes = new List<AForge.Math.Complex[][]>(); // dynamic list of hrtf, following "network" in SDN script
    ...
    void Update()
    {
        ...
        //Debug.Log(Vector3.Distance(prevSourcePos, this.transform.position));
    ...
    public void hybridSnowman(AForge.Math.Complex[][] hrtf, AForge.Math.Complex[] snow_l, AForge.Math.Complex[] snow_r)
    {
        ...
        float[] As_l = new float[nfreq + 1];
        float[] As_r = new float[nfreq + 1];
        int i, j;
        for (i = 0; i <= nfreq; i++)
        {
            if (i <= idx_low) // snowman
            {
                As_l[i] = Mathf.Log10((float)snow_l[i].Magnitude);
                As_r[i] = Mathf.Log10((float)snow_r[i].Magnitude);
            }
            else if (i >= idx_high) // measured
            {
                As_l[i] = Mathf.Log10((float)hrtf[0][i].Magnitude);
                As_r[i] = Mathf.Log10((float)hrtf[1][i].Magnitude);
            }
            else // cross-fade
            {
                As_l[i] = Mathf.Log10((float)snow_l[i].Magnitude) + (Mathf.Log10((float)hrtf[0][i].Magnitude) - Mathf.Log10((float)snow_l[i].Magnitude)) / (omega[idx_low] - omega[idx_high]);
                As_r[i] = Mathf.Log10((float)snow_r[i].Magnitude) + (Mathf.Log10((float)hrtf[1][i].Magnitude) - Mathf.Log10((float)snow_r[i].Magnitude)) / (omega[idx_low] - omega[idx_high]);
            }
        }
        ...
        while (i < hrtf.Length)
        {
            ...
            while (i <= nfreq)
            {
                ...
                hrtf_direct[0][i].Re = Mathf.Pow(As_l[i], 10) * Mathf.Cos((float)snow_l[i].Phase);
                hrtf_direct[0][i].Im = Mathf.Pow(As_l[i], 10) * Mathf.Sin((float)snow_l[i].Phase);
                hrtf_direct[1][i].Re = Mathf.Pow(As_r[i], 10) * Mathf.Cos((float)snow_r[i].Phase);
                hrtf_direct[1][i].Im = Mathf.Pow(As_r[i], 10) * Mathf.Sin((float)snow_r[i].Phase);
                i++;
            }
            // after Nyquist freq -> symetric of first half
            hrtf_direct[0][i].Re = Mathf.Pow(As_l[As_l.Length - j], 10) * Mathf.Cos((float)snow_l[i].Phase);
            hrtf_direct[0][i].Im = Mathf.Pow(As_l[As_l.Length - j], 10) * Mathf.Sin((float)snow_l[i].Phase);
            hrtf_direct[1][i].Re = Mathf.Pow(As_r[As_r.Length - j], 10) * Mathf.Cos((float)snow_r[i].Phase);
            hrtf_direct[1][i].Im = Mathf.Pow(As_r[As_r.Length - j], 10) * Mathf.Sin((float)snow_r[i].Phase);
            i++;
            j++;
        }

        //// ifft
        //AForge.Math.FourierTransform.FFT(hrtf_snowman_direct[0], AForge.Math.FourierTransform.Direction.Backward);
        //AForge.Math.FourierTransform.FFT(hrtf_snowman_direct[1], AForge.Math.FourierTransform.Direction.Backward);
    }
    ...
    public int[] getIndices(float azimuth, float elevation) // get the matrix indices relative to azimuth (0) and elevation (1)
    {
        ...
        float[] azimuths2 = new float[25];
        float[] elevations2 = new float[50];

        // [el_err, el_idx] = min(abs(elevations-rad2deg(elevation))); --- Matlab
        // [az_err, az_idx] = min(abs(azimuths-rad2deg(azimuth)));
        for (int i = 0; i < 25; i++) // Is there a way to do it more efficiently than a for loop? 
        {
    ...
    public float[] getAzElInteraural(Vector3 sourcePos) // returns array of azimuth, elevation with heading and elevation without heading in interaural-polar coordinate system
    {
        ...
        if (sWitch)
            azEl[1] = 360 - azEl[1];  // this trick works for normal human head movements... Local y axes is the key.. But no time...
